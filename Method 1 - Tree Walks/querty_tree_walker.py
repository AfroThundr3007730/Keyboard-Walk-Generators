...
usage:
    qwerty_tree_walker.py [-h] [-l [L]] [-p [P]] [-x] [-H]
...
class QwertyTreeWalker:
    ...
    def __init__(
        ...
        try:
            with open(graph_data, "r", encoding="utf8") as self.fin:
                self.graph = json.load(self.fin)
        except IOError as e:
    ...
    def start_workers(self, num_of_processes=1, max_depth=1):
        ...
        work_chunks = [
            letters[x : x + self.num_workers]
            for x in range(0, len(letters), self.num_workers)
        ]
        ...
        if not self.stdout:
            print("\n\n" + "*" * 70)
            print("*" * 17 + " WARNING: This may take a while " + "*" * 21)
            print("*" * 17 + " Type: [S]tatus [Q]uit " + "*" * 30)
            print("*" * 70 + "\n\n")
            print(
                "[ "
                + str(self.max_depth)
                + "-step walk STARTED at:\t"
        ...
        if not self.stdout:
            ...
            )  # Listen for user input during run
            t.daemon = True
            t.start()
        while self.exit is False and self.kill is False:
    ...
    def print_end_stats(self, end_time):
        ...
        for stat in iter(self.stats.keys()):
            ...
            _start_time = self.stats[stat]["start_time"]
        avg_walk_rate = walk_rate / self.chunks
        print("[Done]")
        print("\n\t[Run Stats]")
        print(
            "\t\tElasped Time: "
            + str((end_time - self.start_time) / 60)
            + " minutes"
        )
        print("\t\t" + f"{avg_walk_rate:.8f}" + " walks/sec/worker")
    ...
    def print_inc_stats(self):
        ...
        for stat in iter(self.stats.keys()):
            ...
            _start_time = self.stats[stat]["start_time"]
        print(
            f"\r{walk_rate:.8f} walks/sec\t Walks: {walks_generated} Walks Left: {walks_left}"
        )
        sys.stdout.write("\r")
        sys.stdout.flush()
    ...
    def start_walking(self, max_depth, pid, letters=None):
        ...
        fout = open(out_file, "a", encoding="utf8")
        ...
        for letter in letters:
            ...
            if file_size > 524288000:
                ...
                sub_file += 1
                out_file = (
                    out_file[: len(out_file) - 6] + "-" + str(sub_file) + ".txt"
                )
                fout = open(out_file, "a", encoding="utf8")
        fout.flush()
        fout.close()
    ...
    def walk(
        ...
        if current_depth >= max_depth:
            ...
            ascii_hash = ""
            if self.show_hash:  # Handles the -H arg
                hashalg = hashlib.new(
                    "md4",  # DevSkim: ignore DS126858
                    plain_text.encode("utf-16le"),
                ).digest()
                ascii_hash = binascii.hexlify(hashalg)
            if self.no_plain:  # Handles the --noplain
                plain_text = ""
            else:
                plain_text = plain_text + "\t"
            if not self.stdout:
                fout.write(plain_text + str(ascii_hash))
                fout.write("\n")
                fout.flush()
            else:
                print(plain_text + str(ascii_hash))
            ...
            for direction in self.graph[current_node]:
                ...
                if not direction in self.exclude_list:
                    walks_generated = self.walk(
                        ...
                        next_level,
                        max_depth,
                        self.graph[current_node][direction],
                        total_walks,
                        path_stack,
                        walks_generated,
                    )
    ...
    def walks_to_completion(self, walk_length, graph):
        ...
        where, e = # of edges
               ...
               n = length of walk
        """
        num_of_edges = len(self.graph[self.graph.keys()[0]]) - len(
            self.exclude_list
        )  # Grab the first element and count links
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate walks for Qwerty Keyboard"
    )
    parser.add_argument(
        "file_name",
        nargs="?",
        help="File with adjacency list of format {'letter': {'direction': 'letter connected'}}",
        type=str,
    )
    parser.add_argument(
        "-l", "-length", nargs="?", help="Walk length", type=int, default=2
    )
    parser.add_argument(
        "-p",
        "-processes",
        nargs="?",
        help="Number of processses to divide work",
        type=int,
        default=1,
    )
    parser.add_argument(
        "-x",
        "-exclude",
        help="Will trigger prompt for link exclude list",
        action="store_true",
    )
    parser.add_argument(
        "-H",
        "-hash",
        help="Output NTLM hash for testing",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--stdout", help="Output to screen", action="store_true", default=False
    )
    parser.add_argument(
        "--noplain",
        help="Do not print plain text hash",
        action="store_true",
        default=False,
    )
    ...
    walker = QwertyTreeWalker(
        args.file_name, args.x, args.noplain, args.H, args.stdout
    )
