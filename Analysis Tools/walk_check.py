...
positional arguments:
    graph_file_name     File with adjacency list of format:
        ...
        if i < len(password) - 1:
            ...
            if current_letter in graph:
                if loop:
                    ...
                    if (
                        next_letter in graph[current_letter].values()
                        and next_letter.lower() != current_letter.lower()
                    ):
                        path_length += 1
                        if path_length == length:
                            result = True
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Check if string(s) are keyboard walks"
    )
    parser.add_argument(
        "graph_file_name",
        nargs="?",
    ...
    parser.add_argument(
        "input", nargs="?", help="File name or single string to check", type=str
    )
    parser.add_argument(
        "-l", "-length", nargs="?", help="Walk length", type=int, default=4
    )
    parser.add_argument(
        "-strict",
        help="Only find exact walks of length specified by -l option",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "-loop",
        help="Consider adjacent dublicate letters as walks",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "-stats",
        help="Do some calculations",
        action="store_true",
        default=False,
    )
    args = parser.parse_args()
    try:
        with open(args.graph_file_name, "r", encoding="utf8") as fin:
            graph = json.load(fin)
    except IOError as e:
    ...
    if os.path.isfile(args.input):
        with open(args.input, "r", encoding="utf8") as fin:
            ...
            if args.stats:
                ...
                print(
                    "Total Walks:\t " + str(total_true),
                    f"({float(total_true) / float(total) * 100:.1f}%)",
                )
    else:
        status = walk_checker(
            graph, args.input.rstrip("\n\r"), args.l, args.strict, args.loop
        )
        if status:
